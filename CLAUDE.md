...
- ✅ 홈 페이지 (`/`) - 가챠 머신 UI, "노래 뽑기" 버튼
- ✅ 추천 페이지 (`/recommendations`) - 15곡 랜덤 추천 결과
- ✅ 가챠 머신 애니메이션 (float, shake, shadow)
- ✅ Supabase 연동 - `get_random_songs` RPC 함수 호출
- ✅ Clean Architecture 구조 (Domain → UseCase → Repository → Supabase)
- ✅ React Query를 통한 서버 상태 관리 + Prefetch
- ✅ Zod 4를 통한 DTO 검증 및 도메인 매핑
- ✅ 노래 카드 UI (앨범아트, 제목, 아티스트, Spotify 딥링크)
- ✅ "다시 뽑기" 기능 (refetch)
- ✅ 긴 텍스트 자동 스크롤 (MarqueeText)
- ✅ TDS Mobile UI 컴포넌트 적용
...
│   └── song/
│       ├── index.ts             # Barrel export (SongItem, SongList, hooks)
│       ├── actions/             # ⚠️ DI Container 의존 허용 (진입점)
│       │   └── getRandomSongsAction.ts
│       ├── hooks/               # ⚠️ React Query 의존 허용
...
│       │   ├── SongList.tsx     # 노래 목록 (로딩/에러/빈 상태 처리)
│       │   └── SongItem.tsx     # 노래 카드 (앨범아트, MarqueeText)
│       └── usecase/             # ✅ 순수 비즈니스 로직
│           └── getRandomSongs.ts
│
├── infrastructure/               # 인프라 레이어 (외부 시스템 연동)
│   ├── adapters/                # Repository 구현체
│   │   └── SongRepository.ts    # Supabase 어댑터 + Zod 검증
│   ├── di/                      # 의존성 주입
│   │   └── container.ts         # DI 컨테이너 (싱글톤)
│   ├── lib/                     # 외부 라이브러리 클라이언트
...
│   │   └── TDSProvider.tsx
│   └── schemas/                 # Zod 스키마
│       └── song.ts              # SongDTO 검증 & toDomain 매핑
│
├── view/                         # View 레이어 (복합 UI)
│   └── widgets/
...
UI (SongList, GachaMachine)
        ...
        ▼
React Hooks (useRandomSongs, usePrefetchRandomSongs)
        │ TanStack Query
        ▼
Action (getRandomSongsAction)
        │ DI Container
        ▼
...
Repository (SongRepository)
        │ Zod Validation (SongDTOListSchema)
        ▼
...
1. **Dependency Injection** - `container.ts`에서 싱글톤 인스턴스 관리
2. **Repository Pattern** - `ISongRepository` 인터페이스로 데이터 접근 추상화
3. **Use Case Pattern** - 단일 책임 비즈니스 로직 클래스
4. **React Query** - 서버 상태 관리 + Prefetch (가챠 애니메이션 중 데이터 로딩)
5. **Zod Validation** - DTO → Domain 매핑 시 검증 (snake_case → camelCase)
6. **CVA** - 컴포넌트 variant 타입 안전 관리 (size, state)
7. **Barrel Exports** - `index.ts`를 통한 깔끔한 모듈 인터페이스

## 아키텍처 결정 (ADR)

### 실용적 타협: Features에서 Infrastructure 의존 허용

**문제**: Clean Architecture 원칙대로라면 `features → infrastructure` 의존은 위반

**결정**: 소규모 프로젝트에서 실용성을 위해 **일부 예외 허용**

**허용된 의존성**:

| 위치                | 의존 대상                     | 이유                 |
| ------------------- | ----------------------------- | -------------------- |
| `features/actions/` | `infrastructure/di/container` | 진입점으로서 DI 필요 |
| `features/hooks/`   | `@tanstack/react-query`       | React 생태계의 표준  |
...
| ------------------- | --------------- |
| `domain/`           | 없음 (순수)     |
| `features/usecase/` | domain, ports만 |
| `features/ports/`   | domain만        |

**근거**:

- 100% Clean Architecture 적용 시 과도한 추상화 발생
- Next.js 생태계에서 actions/hooks는 기능 단위로 묶는 것이 자연스러움
- 핵심 비즈니스 로직(UseCase)만 순수하면 테스트/유지보수에 충분

```
실제 의존성 흐름:
app → features (actions, hooks, ui) → infrastructure (di, adapters)
              ↓
         usecase, ports → domain
```
...
- GachaMachine 컴포넌트 (floating 애니메이션)
- "노래 뽑기" 버튼 클릭
- `usePrefetchRandomSongs(15).prefetch()` 호출 (백그라운드 데이터 로딩)
- 1700ms 동안 shake 애니메이션
- `/recommendations`로 라우팅

### 2. 추천 페이지 (`/recommendations`)

- `useRandomSongs(15)` 훅으로 캐시된 데이터 사용 (이미 prefetch됨)
- SongList로 15곡 표시
- 각 노래: 앨범아트 (48x48), 제목/아티스트 (MarqueeText), Spotify 아이콘
- Spotify 아이콘 클릭 → 새 탭에서 Spotify 열기
- "다시 뽑기" 버튼 → `refetch()` + shake 애니메이션 (1700ms)

## React Query 훅 사용법

```typescript
// 데이터 조회
const { data: songs, isLoading, error, refetch } = useRandomSongs(15);

// Prefetch (가챠 애니메이션 중 백그라운드 로딩)
const { prefetch } = usePrefetchRandomSongs(15);
prefetch(); // 호출 시 백그라운드에서 데이터 로딩
```
...
- **Colors**: `@toss/tds-colors` (adaptive 컬러 시스템)
...
| Zod 스키마            | `src/infrastructure/schemas/song.ts`                |
| React Query 훅        | `src/features/song/hooks/useRandomSongs.ts`         |
| Query Key Factory     | `src/features/song/hooks/keys.ts`                   |
| Action                | `src/features/song/actions/getRandomSongsAction.ts` |
| Feature Barrel        | `src/features/song/index.ts`                        |
...
- [ ] Sentry 에러 로깅 연동 (현재 console.error 사용)
- [ ] 노래 즐겨찾기/저장 기능
- [ ] 장르별 필터링
- [ ] 노래 검색 기능
- [ ] 사용자별 추천 히스토리
